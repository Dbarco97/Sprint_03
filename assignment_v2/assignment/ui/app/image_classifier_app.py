...
def login(username: str, password: str) -> Optional[str]:
    ...
    try:
        ...
        if response.status_code == 200:
            # Step 6: Extract the token from the response JSON
            token = response.json().get("access_token")
            return token
        else:
            st.error(f"Login failed: {response.status_code} {response.reason}")
    except requests.RequestException as e:
        st.error(f"An error occurred: {str(e)}")

    # Step 7: Return None if the login fails
    return None
def predict(token: str, uploaded_file: Image) -> requests.Response:
    ...
    predict_endpoint=API_BASE_URL + "/model/predict"
...
def send_feedback(
    ...
    feedback_data={"feedback":feedback,"score":score, "predicted_class":prediction,"image_file_name":image_file_name}
    headers={"Authorization":f"Bearer {token}"}
