...
def predict(image_name):
    ...
    _, class_name, pred_probabilities=decoded_preds[0]

    # Convert probabilities to float and round it
    pred_probabilities=round(float(pred_probabilities),4)
...
def classify_process():
    ...
    while True:
        ...
        predicted_name, pred_score= predict(image_name)
        # Prepare a new JSON with the results
        output = {"prediction": predicted_class, "score": pred_score}
